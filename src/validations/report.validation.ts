...
export const SCHEMA_LIST_REPORT_QUERY = Joi.object({
  ...
  urlCheckId: Joi.string().required(), // in list options not in query params for future minimum changes if we add another entity and want to fetch the reports with this new entity => just add its id here not new api required for it and less services of course
}).optional();
